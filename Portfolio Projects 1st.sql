Select continent, max(total_deaths) as total
...
Select location, date, total_cases, total_deaths, cast (total_deaths/total_cases as decimal(15,4))*100 as DeathRatio
From PortfolioProject..CovidDeaths
...
Select Location, Date, Population, Total_cases, (total_cases/population)*100 as 'Total Population Infected'
From PortfolioProject..CovidDeaths
Where location like 'Greece' AND date >= '2020-03-01 00:00:00.000'
Order by 1,2


-- Looking at Countries with Highest Infection Rate compared to Population
Select location, Population, MAX(Total_cases) AS HighestInfectionCount, MAX((total_cases/population))*100 as HighestInfectionRatio
From PortfolioProject..CovidDeaths
Group by location, population
Order by HighestInfectionRatio desc

-- Showing Countries with Highest Death Count per Population
Select Location, Population, MAX(total_deaths) AS TotaltDeathCount, MAX(total_deaths/population)*100 as HighestDeathRatio
From PortfolioProject..CovidDeaths
Where continent is not NULL
Group by location, population
Order by HighestDeathRatio desc

-- Show things by Location/ null Continent
...
Select Continent, MAX(total_deaths) AS TotaltDeathCount, MAX(total_deaths/population) as 'Deaths/Population'
From PortfolioProject..CovidDeaths
Where continent is not NULL
Group by Continent
Order by TotaltDeathCount desc
...
--GLOBAL NUMBERS
Select SUM(TOTAL_CASES) AS 'Total Cases', SUM(new_cases) AS 'Total New Cases', SUM(new_deaths) AS 'Total New Deaths', SUM(new_deaths)/SUM(new_cases)*100 as 'Death Percentage WW'
From PortfolioProject..CovidDeaths
...
Select *, (TotalVaccinationsInCountry/population)*100 AS PercentagePopulationVac
From PopvsVac
